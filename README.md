...
**Android-приложение для работы с моделями OpenRouter, проектами и командой специализированных ИИ-чатов.**
...
Umnik объединяет обычные чаты, рабочие проекты, локальные навыки и базы знаний. В одном проекте можно создать несколько ИИ-специалистов с разными моделями и инструкциями, а чат **◆ Оркестратор** свяжет их в управляемую рабочую цепочку и передаст между ними результаты и файлы.

**[Скачать последний стабильный релиз](https://github.com/Ayuemin/Umnik/releases/latest)**

## Что можно делать

- общаться с текстовыми и мультимодальными моделями OpenRouter;
- создавать отдельных ИИ-помощников для исследования, анализа, написания и проверки;
- объединять помощников в проекты с общими правилами, файлами и базой знаний;
- поручать Оркестратору многошаговые задачи обычной фразой;
- хранить чаты, проекты, навыки и индексы базы знаний локально на Android;
- запускать несколько чатов параллельно и переключаться между ними во время ответа;
- использовать изображения, аудио, видео, Batch, Shell, веб-поиск и reasoning там, где их поддерживает выбранная модель.

Umnik не предоставляет собственную подписку на нейросети. Для работы нужен личный API-ключ OpenRouter; стоимость запросов зависит от выбранных моделей.

## Быстрый старт

1. Скачайте APK на странице [последнего релиза](https://github.com/Ayuemin/Umnik/releases/latest).
2. Установите приложение на Android 8.0 или новее.
3. Откройте **Настройки → OpenRouter**, добавьте API-ключ и проверьте подключение.
4. Выберите текстовую модель и начните обычный чат либо создайте проект.

APK, контрольная сумма и отпечаток постоянного сертификата публикуются в Assets каждого официального релиза.
...
Главная идея проста: **модель — только одна часть специалиста**. В Umnik специалист складывается из модели, роли, инструкций, навыков, инструментов, истории, файлов и базы знаний.
...
- OpenRouter `chat/completions`, Responses API для совместимых инструментов и каталог моделей;
- потоковое получение текста и фоновое выполнение активных запросов;
...
Поддержка reasoning, изображений, аудио, видео, tools и других возможностей зависит от конкретной модели OpenRouter.
...
При обычных запросах данные отправляются через OpenRouter выбранной модели. При индексации базы знаний текстовые фрагменты отправляются в OpenRouter для вычисления embeddings, после чего исходник, фрагменты, векторы и индекс хранятся локально. При поиске по базе в OpenRouter отправляется текст поискового запроса для получения embedding, а основной модели затем передаются только найденные фрагменты.
