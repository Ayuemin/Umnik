...
s = sub_once(
    ...
    new_tools,
    "replace skills/projects composer block",
)
...
''',
    ...
    var soundExpanded by remember { mutableStateOf(false) }
''',
    "settings skill state",
)

skill_manager = r'''@Composable
...
s = replace_once(
    ...
    skill_manager + '@Composable\nprivate fun SettingsScreen(state: UiState, vm: ChatViewModel, onBack: () -> Unit) {',
    "insert skill library settings",
)
...
s = replace_once(
    ...
    "projects direct create",
)

new_project_detail = r'''@Composable
...
s = sub_once(
    ...
    new_project_detail + '@Composable\nprivate fun ProjectEditorDialog(',
    "replace project detail/settings",
)
path.write_text(s)
...
s = path.read_text()
s = s.replace('// Umnik v1.10.8', '// Umnik v1.10.9', 1)
s = s.replace('versionCode = 108', 'versionCode = 109', 1)
...
'''
if entry.splitlines()[0] not in s:
