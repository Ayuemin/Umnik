...
# Experiment only: make chat/completions non-streaming so the background test does not
# depend on a long-lived SSE body. This lets us distinguish an SSE-specific disconnect
# from Android/OEM killing any long-running network request.
needle = """        val requestPayload = payload.deepCopy().apply {\n            addProperty(\"stream\", true)\n            add(\"stream_options\", JsonObject().apply { addProperty(\"include_usage\", true) })\n        }\n"""
replacement = """        val requestPayload = payload.deepCopy().apply {\n            addProperty(\"stream\", false)\n            remove(\"stream_options\")\n        }\n"""
if needle not in text:
    raise SystemExit("non-streaming experiment insertion point not found")
